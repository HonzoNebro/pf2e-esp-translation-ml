...
Hooks.once("init", () => {
    ...
    game.settings.register("pf2e-esp-translation-ml", "token", {
...
class Translator {
    ...
    async initialize() {
        ...
        const config = await Promise.all([
            fetch("modules/pf2e-esp-translation-ml/src/translator/translator-config.json")
                .then((r) => r.json())
                .catch((_e) => {
                    console.error("pf2e-esp-translation-ml: Couldn't find translator config file.");
                }),
        ]);
        ...
        if (dictionaryPath) {
            ...
            console.error("pf2e-esp-translation-ml: Dictionary not available");
        }
    ...
    addMediaPath(compendium, path) {
        ["portraits", "tokens"].forEach(async (imageType) => {
            const imagePath = game.settings.get("pf2e-esp-translation-ml", "token")
    ...
    registerCompendium(module, compendium, language, compendiumDirectory, imageDirectory = undefined) {
        ...
        if (game.babele) {
            ...
            console.error("pf2e-esp-translation-ml: Required module Babele not active");
        }
    ...
    translateDualLanguage(data, translation) {
        if (!translation || data === translation) {
            return data;
        } else if (game.settings.get("pf2e-esp-translation-ml", "dual-language-names")) {
